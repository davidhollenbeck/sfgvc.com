...
add_action( 'wp_enqueue_scripts', 'sfg_products_match_height', 99 );
/**
 * Print an inline script to the footer to keep products the same height.
 *
 * @since 2.3.0
 */
function sfg_products_match_height() {
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'sfg_woocommerce_breakpoint' );
/**
 ...
 */
function sfg_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'sfg_default_products_per_page' );
/**
 ...
 */
function sfg_default_products_per_page() {
	return 8;
}

add_filter( 'woocommerce_pagination_args', 	'sfg_woocommerce_pagination' );
/**
 ...
 */
function sfg_woocommerce_pagination( $args ) {
...
add_action( 'after_switch_theme', 'sfg_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
* Define WooCommerce image sizes on theme activation.
*
* @since 2.3.0
*/
function sfg_woocommerce_image_dimensions_after_theme_setup() {
	...
	sfg_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'sfg_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 * Define the WooCommerce image sizes on WooCommerce activation.
 *
 * @since 2.3.0
 */
function sfg_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function sfg_update_woocommerce_image_dimensions() {
