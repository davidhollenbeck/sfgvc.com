...
add_action( 'admin_print_styles', 'sfg_remove_woocommerce_notice' );
/**
 * Remove the default WooCommerce Notice.
 *
 * @since 2.3.0
 */
function sfg_remove_woocommerce_notice() {
...
add_action( 'admin_notices', 'sfg_woocommerce_theme_notice' );
/**
 ...
 * @since 2.3.0
 */
function sfg_woocommerce_theme_notice() {
	...
	if ( get_user_option( 'sfg_woocommerce_message_dismissed', get_current_user_id() ) ) {
		return;
	}
...
add_action( 'wp_ajax_sfg_dismiss_woocommerce_notice', 'sfg_dismiss_woocommerce_notice' );
/**
 * Add option to dismiss Genesis Connect for Woocommerce plugin install prompt.
 *
 * @since 2.3.0
 */
function sfg_dismiss_woocommerce_notice() {
	update_user_option( get_current_user_id(), 'sfg_woocommerce_message_dismissed', 1 );
}

add_action( 'admin_enqueue_scripts', 'sfg_notice_script' );
/**
 * Enqueue script to clear the Genesis Connect for WooCommerce plugin install prompt on dismissal.
 *
 * @since 2.3.0
 */
function sfg_notice_script() {
	wp_enqueue_script( 'sfg_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', array( 'jquery' ), '1.0', true  );
}

add_action( 'switch_theme', 'sfg_reset_woocommerce_notice', 10, 2 );
/**
 * Clear the Genesis Connect for WooCommerce plugin install prompt on theme change.
 *
 * @since 2.3.0
 */
function sfg_reset_woocommerce_notice() {

	global $wpdb;

	$args = array(
		'meta_key'   => $wpdb->prefix . 'sfg_woocommerce_message_dismissed',
		'meta_value' => 1,
	);
	$users = get_users( $args );

	foreach ( $users as $user ) {
		delete_user_option( $user->ID, 'sfg_woocommerce_message_dismissed' );
	}

}

add_action( 'deactivated_plugin', 'sfg_reset_woocommerce_notice_on_deactivation', 10, 2 );
/**
 ...
 */
function sfg_reset_woocommerce_notice_on_deactivation( $plugin, $network_activation ) {
